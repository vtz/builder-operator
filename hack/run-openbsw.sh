#!/usr/bin/env bash
# Run the Eclipse OpenBSW posix-freertos build pipeline via the Builder Operator.
...
CR_NAME="openbsw-posix-freertos"
...
kubectl apply -f "${ROOT_DIR}/docs/examples/openbsw-posix-freertos.yaml"
...
echo "Waiting for pipeline to complete (up to 60 min — CMake build + FreeRTOS dependency download)..."
kubectl wait --for=condition=Succeeded "pipelinerun/${RUN}" -n "${NAMESPACE}" --timeout=3600s || true
...
echo "Artifacts:"
ls -lR "${ROOT_DIR}/deployment/openbsw-posix-freertos/" 2>/dev/null || echo "No artifacts found"
